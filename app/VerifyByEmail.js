...
export default function VerifyByEmail() {
    ...
    function maskEmail(email) {
        ...
        // If username is less than 2 characters, we can't mask much, so just return the original email.
        if (username.length < 2) {
            return email;
        }

        // Mask the username by keeping the first and last character and replacing the rest with "*"
    ...
    const [getEmail, setEmail] = useState(maskEmail(userObject.email));
    ...
    async function sendOtp() {
        ...
        let response = await fetch(
            "http://192.168.8.187:8080/CyberChat/SendEmailOtp",
            {
                method: "POST",
        ...
        <SafeAreaView style={stylesheet.container}>
            <View style={stylesheet.body}>
                ...
                <View style={stylesheet.titleView}>
                    <Text style={stylesheet.title}>Check your email</Text>
                </View>
                <Text style={stylesheet.mainText}>We sent your code to
                    <Text style={stylesheet.textMobile}>{" " + getEmail + " "}</Text>
                </Text>
                ...
                <View style={stylesheet.bodyMain}>
                    <View style={stylesheet.bodySub1}>
                        ...
                        <Text style={[stylesheet.subText, { color: status == "Running" ? "gray" : "blue" }]} onPress={() => {
                            if (status == "Complete") {
                                sendOtp();
                            }
                        }} disabled={status == "Running" ? true : false}> Didn't receive code?</Text>
                        <Text style={stylesheet.textCountDownDesc} onPress={() => {

                        }}>You may request a new code in <Text style={stylesheet.textCountDown} >{time}</Text></Text>
                        ...
                        <Pressable style={stylesheet.btn} onPress={
                            () => {
                                ...
                                if (finalOtp.length === 6) {
                                    ...
                                    if (finalOtp == getEmailOtp) {

                                        console.log("Match found: " + getEmailOtp + " OK");
                                        userObject.emailOtp = getEmailOtp;
                                        console.log(userObject);

                                        async function SaveEmail() {

                                            let response = await fetch("http://192.168.8.187:8080/CyberChat/SaveEmail",
                                                {
                                                    method: "POST",
                                                    body: JSON.stringify(userObject)
                                                }
